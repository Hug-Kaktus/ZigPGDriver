...
test "simple query untyped" {
    ...
    defer conn.close();
    const result = try queryUntyped(&conn, "SELECT employee_id, first_name, last_name FROM employee");
...
test "drop table" {
    ...
    defer conn.close();
    _ = try queryUntyped(&conn, "DROP TABLE employee;");
}
...
test "extended query untyped" {
    ...
    const prepared_statement = try prepare(&conn, "test", "SELECT employee_id, first_name, last_name FROM employee WHERE employee_id = $1", &param_types);
